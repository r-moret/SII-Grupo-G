...
public class TestMatriculaEJB {
	...
	public void testConsultarMatricula() {
		...
		try {
			...
			fail("Lanza la excepci�n incorrecta 1111");
		}
		...
		try {
			...
			fail("Lanza la excepci�n incorrecta 2322");
		}
		...
		try {
			...
			assertTrue("El m�todo devuelve Null en vez de una lista vac�a",
					matriculaEJB.consultarMatricula(expediente).isEmpty());

		} catch (Exception e) {
			fail("El m�todo lanza una excepci�n cuando el comportamiento es correcto 1111");
		}
		...
		try {
			assertTrue("El m�todo no devuelve una lista de matr�culas correcta",
					expediente2.getMatriculas().equals(matriculaEJB.consultarMatricula(expediente2)));
		} catch (Exception e) {
			fail("El m�todo lanza una excepci�n al comprobar un expediente correcto 2222");
		}
	}
	...
	public void testBuscarMatriculaPorCurso() {
		...
		try {
			...
			// No falla
			fail("Permite consultar un expediente por un curso acad�mico nulo");
		} catch (CursoInexistente exc1) {
			/* Falla y lanza la excepcion correcta */} catch (Exception e) {
			fail("Lanza la excepci�n incorrecta");
		}

		// Caso 2 - paso un curso académico que no está en ninguna matrícula del
		// expediente
		Expediente exp = em.find(Expediente.class, 8);
		try {
			...
			fail("Lanza una excepci�n incorrecta");
		}
		try {
